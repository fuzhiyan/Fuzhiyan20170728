...
//            Glide.with(context).load(list.get(position).getUserImg()).into(vh.userImage);
        ...
        x.image().bind(vh.userImage,list.get(position).getUserImg());
        vh.userName.setText(list.get(position).getUserName());
//        vh.userAge.setText(list.get(position).getUserAge());
        vh.occupation.setText(list.get(position).getOccupation());
        vh.introduction.setText(list.get(position).getIntroduction());

        vh.userImage.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Toast.makeText(context, ""+list.get(position).getIntroduction(), Toast.LENGTH_SHORT).show();
            }
        });
